...
struct ssh_tag {
    ...
    struct ssh_channel *secondary_chan;
...
{
	if (ssh->secondary_chan != NULL && ssh->secondary_chan->localid == ssh_pkt_getuint32(pktin)) {
		ssh->secondary_chan->remoteid = ssh_pkt_getuint32(pktin);
		ssh->secondary_chan->halfopen = FALSE;
		ssh->secondary_chan->type = CHAN_SECONDARY;
...
static void ssh2_response_command(struct ssh_channel *c, struct Packet *pktin, void *ctx)
{
	char msgbuf[256];
	sprintf(msgbuf, "%s", pktin->data);
...
static Socket ssh_get_socket(void *handle)
{
    Ssh ssh = (Ssh) handle;
    return ssh->s;
}
